...
<div class="home-page">
    {% assign kit = site.data.speaker_kit %}
    {% assign all_talks = site.talks %}
    {% assign talks_with_video = all_talks | where_exp: "t", "t.extracted_video" %}
    {% assign event_names = all_talks | map: "extracted_conference" | compact | uniq %}

    <header class="speaker-hero speaker-bar">
        <div class="speaker-hero__text speaker-bar__info">
            {% if site.speaker.badges %}
            <ul class="badge-list" aria-label="Highlights">
                {% for badge in site.speaker.badges %}<li class="badge">{{ badge }}</li>{% endfor %}
            </ul>
            {% endif %}
            ...
            {% else %}
                <h1>Speaker</h1>
            {% endif %}
            {% if site.speaker.headline %}
                <p class="speaker-hero__headline">{{ site.speaker.headline }}</p>
            {% endif %}
            {% if site.speaker.intro and site.speaker.intro != "" %}
                <p class="speaker-bar__bio speaker-hero__bio">{{ site.speaker.intro }}</p>
            {% elsif site.speaker and site.speaker.bio and site.speaker.bio != "" %}
                <p class="speaker-bar__bio speaker-hero__bio">{{ site.speaker.bio }}</p>
            {% endif %}
            <div class="speaker-hero__actions">
                {% if site.speaker.booking_url and site.speaker.booking_url != "" %}
                <a class="btn btn--primary" href="{{ site.speaker.booking_url }}">Invite me to speak</a>
                <a class="btn btn--secondary" href="#all-talks">Browse talks</a>
                {% else %}
                <a class="btn btn--primary" href="#all-talks">Browse talks</a>
                {% endif %}
                <a class="btn btn--secondary" href="{{ '/bio/' | relative_url }}">Bio &amp; press kit</a>
            </div>
        ...
        {% if kit.hero %}
        <div class="speaker-hero__photo">
            <img src="{{ '/assets/images/speaker/' | append: kit.hero.preview | relative_url }}" alt="{{ kit.hero.alt }}" width="{{ kit.hero.width }}" height="{{ kit.hero.height }}" fetchpriority="high">
        </div>
        {% else %}
        {% assign avatar_url = "" %}
        {% if site.speaker.social.github and site.speaker.social.github != "" %}
        ...
        {% endif %}
        {% if avatar_url != "" %}
        <div class="speaker-hero__photo"><img src="{{ avatar_url }}" alt="{{ site.speaker.name }}" class="speaker-bar__avatar"></div>
        {% endif %}
        {% endif %}
    </header>

    {% if all_talks.size > 0 %}
    <dl class="speaker-stats" aria-label="Speaking in numbers">
        <div class="speaker-stats__item"><dt>Talks</dt><dd>{{ all_talks.size }}</dd></div>
        <div class="speaker-stats__item"><dt>Recorded</dt><dd>{{ talks_with_video.size }}</dd></div>
        <div class="speaker-stats__item"><dt>Events</dt><dd>{{ event_names.size }}</dd></div>
        {% comment %} The archive's first year understates a long career, so only show a year the speaker sets {% endcomment %}
        {% if site.speaker.speaking_since and site.speaker.speaking_since != "" %}<div class="speaker-stats__item"><dt>On stage since</dt><dd>{{ site.speaker.speaking_since }}</dd></div>{% endif %}
    </dl>
    {% endif %}

    {% if site.featured_conferences and site.featured_conferences.size > 0 %}
    <section class="featured-at" aria-labelledby="featured-at-heading">
        <h2 id="featured-at-heading" class="featured-at__label">Featured at</h2>
        <ul class="featured-at__list">
            {% for conference in site.featured_conferences %}<li>{{ conference }}</li>{% endfor %}
        </ul>
    </section>
    {% endif %}
    ...
    {% if talks.size > 0 %}
        ...
        {% for talk in talks %}
            ...
            {% endif %}
        {% endfor %}
        {% if talks_with_previews.size == 0 %}
            {% for talk in talks %}
                {% if talk.extracted_slides or talk.thumbnail_url %}
                    {% assign talks_with_previews = talks_with_previews | push: talk %}
                {% endif %}
            {% endfor %}
        {% endif %}
        ...
        <!-- All Talks Section -->
        {% if talks.size > 0 %}
        <section class="all-talks" id="all-talks">
            <h2>All talks</h2>
            {% comment %} Filters are hidden until assets/js/talk-filters.js enhances them {% endcomment %}
            <div class="talk-filters" data-talk-filters hidden>
                <div class="talk-filters__search">
                    <label for="talk-search" class="visually-hidden">Search talks</label>
                    <svg class="talk-filters__search-icon" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>
                    <input id="talk-search" type="search" placeholder="Search talks, events, topics" autocomplete="off" enterkeyhint="search">
                </div>
                <div class="chip-row" role="group" aria-label="Filter by topic">
                    <button type="button" class="chip" aria-pressed="true" data-topic="">All topics</button>
                    {% for topic in site.speaker.topics %}
                    <button type="button" class="chip" aria-pressed="false" data-topic="{{ topic.keywords | join: '|' | escape }}">{{ topic.name }}</button>
                    {% endfor %}
                    <button type="button" class="chip chip--toggle" aria-pressed="false" data-video-only>
                ...
                <p class="talk-filters__status" aria-live="polite" data-talk-count></p>
            </div>
            ...
            <nav aria-label="Jump to year" class="year-nav-wrapper">
                <ul class="year-nav">
                    {% for year in talks_by_year %}
                    <li data-year-link="{{ year.name }}"><a href="#year-{{ year.name }}">{{ year.name }} <span class="year-nav__count">{{ year.size }}</span></a></li>
                    {% endfor %}
                </ul>
            </nav>
            {% for year in talks_by_year %}
            ...
            <div class="talks-list">
                ...
                {% capture search_text %}{{ talk.extracted_title }} {{ talk.extracted_conference }} {{ talk.extracted_description }}{% endcapture %}
                <article class="talk-list-item" data-search="{{ search_text | strip_html | downcase | normalize_whitespace | escape }}"{% if talk.extracted_video %} data-video{% endif %}>
            ...
            {% endfor %}
            <p class="talk-filters__empty" data-talk-empty hidden>No talks match. <button type="button" class="link-button" data-talk-reset>Clear filters</button></p>
        </section>
        {% endif %}

        <section class="kit-teaser" aria-labelledby="kit-teaser-heading">
            <div>
                <h2 id="kit-teaser-heading">Organizing an event?</h2>
                <p>Bios, headshots, and topics are ready to copy and download.</p>
            </div>
            <div class="kit-teaser__actions">
                <a class="btn btn--primary" href="{{ '/bio/' | relative_url }}">Bio &amp; press kit</a>
                {% if site.speaker.booking_url and site.speaker.booking_url != "" %}<a class="btn btn--secondary" href="{{ site.speaker.booking_url }}">Invite me to speak</a>{% endif %}
            </div>
        </section>
